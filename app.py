...
class Tg_api(Base):
    __tablename__ = 'tele_apikey'
    id = Column(Integer, primary_key=True)
    api_id = Column("api_id", Integer)
    api_hash = Column("api_hash", VARCHAR(64))
    phone = Column("phone", VARCHAR(64))
    last_time = Column('last_time', DateTime)
    active = Column('active', VARCHAR(32))

min_time = DBSession.query(func.min(Tg_api.last_time)).filter(Tg_api.active == 'true').first()
#print(min_time[0])
api=DBSession.query(Tg_api.api_id,Tg_api.api_hash,Tg_api.phone).filter(Tg_api.last_time == min_time[0]).first()
#print(api[0], api[1], api[2])
...
date_obj = datetime.now()
stmt2 = (update(Tg_api).where(Tg_api.api_id == api[0]).values(last_time= date_obj))
